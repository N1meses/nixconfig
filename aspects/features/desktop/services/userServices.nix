_: {
  aspects.desktop.services.userServices = {
    description = "Per-user session services: automounting and notifications.";
    home =
      {
        config,
        lib,
      ...
      let
        ...
        udiskieArgs = lib.concatStringsSep " " [
          (if cfg.storage.udiskie.automount then "-a" else "-A")
          (if cfg.storage.udiskie.notify then "-n" else "-N")
          "-T"
        ];
      in
      {
