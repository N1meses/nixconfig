...
in
{
  aspects.desktop.compositors.niri = {
    nixos = { pkgs, ... }: {
      ...
      in
      {
        options.features.compositors.niri = {
          ...
          extraConfig = lib.mkOption {
            ...
            default = [ ];
            description = "Extra raw KDL appended to niri config.kdl (e.g. layer-rules).";
          };
        };
        ...
        config = {
          ...
          xdg.config.files."niri/config.kdl".text =
            inputs.niri-nix.lib.mkNiriKDL settings
            + "\n"
            + lib.concatStringsSep "\n" config.features.compositors.niri.extraConfig;
        };
      };
    description = "The niri scrolling Wayland compositor, with keybinds and portal wiring.";
