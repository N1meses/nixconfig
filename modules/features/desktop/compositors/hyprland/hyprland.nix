{
  ...
  aspects.hyprland = {
    ...
    home = {
      ...
      monitorsBlock =
        ...
        then
          lib.concatStrings (lib.mapAttrsToList (
              name: m: ''hl.monitor({ output = "${name}", mode = "${toString m.resolution.width}x${toString m.resolution.height}@${toString m.refreshRate}", position = "${toString m.position.x}x${toString m.position.y}", scale = ${toString m.scale} })'' + "\n"
            )
            c.monitors)
        else ''hl.monitor({ output = "", mode = "preferred", position = "auto", scale = "auto" })'' + "\n";
      ...
      gestureBlock = lib.optionalString hcfg.input.touchpad.enable ''
