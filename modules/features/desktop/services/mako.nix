{ config, ... }: {
  aspects.desktop.services.mako = {
    description = "mako notification daemon, themed.";
    ...
    home =
      ...
      let
        ...
        toVal = v: if lib.isBool v then (if v then "1" else "0") else toString v;
      in
      {
        packages = [ pkgs.mako ];
        xdg.config.files."mako/config".text = lib.concatStringsSep "\n" (
