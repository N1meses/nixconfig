...
inputs pinned by [pnix](https://forgejo.nimeses.com/nimeses/pnix), built with
`nixos-rebuild --file . --attr`. Home
...
```
nixconfig/
├── default.nix            # Entry point - evalModules over modules/ (pnix-resolved inputs)
...
├── .pnix/                 # pnix: default.nix (what you import), pins.lock.json, eval/
│                          #   generated by `pnix init` - do not edit, do not format
├── modules/
...
```bash
nix-shell                 # or `nix develop` - pnix, nixfmt-tree, sops, age, ssh-to-age,
                          #   deploy-rs, disko, nvd, nix-output-monitor, jq
pnix update               # update all pins (.pnix/pins.lock.json)
pnix update <pin>         # update one pin; the rest keep their locked entry
pnix look                 # what would move, without writing or downloading
...
Inputs are declared **in the module that uses them**, not in one manifest:

```nix
# modules/features/server/media/nixarr.nix
{ config, inputs, ... }:
{
  pins.nixarr = {
    type = "git";
...
`pnix update` greps the tree, imports each candidate with throwing stubs for its
arguments, reads `pins`, and writes `.pnix/pins.lock.json`. `default.nix` turns
that lock into `inputs`, which reach modules through `specialArgs` exactly as
before - only the right-hand side changed.

Three rules, each of which has already caught something:

1. **A `pins` block must not touch the file's arguments.** The collector calls
   the file with stubs that throw on access, so `pins` has to be a literal.
   Everything else in the file can do what it likes; only `.pins` is forced.
2. **`_`-prefixed files cannot declare pins.** importTree skips them, so they
   are *inner* NixOS modules - `pins` there lands in NixOS's option namespace
   and is rejected. That is why `disko` and `impermanence` live in
   `modules/pins.nix`.
3. **`options.pins` must exist** (`modules/options/pnix.nix`), or every
   declaration is an undeclared option. `attrsOf unspecified`, not `raw`, so two
   files declaring the same pin identically merge.

`modules/pins.nix` holds only what has no single consumer: `nixpkgs` (read by
`default.nix` before evalModules runs), things several builders share, the two
`_`-file cases, and three pins with no consumer at all.

**Overrides.** `PNIX_OVERRIDE=finix=/home/nimeses/Projects/nix/finix nh os switch`
points a pin at a working tree for one command. It overrides *resolution*, not
declaration - nothing is re-locked. It does **not** survive `sudo`/`doas`, which
reset the environment, nor a flake's pure evaluation, where `getEnv` returns
empty; use `sudo env PNIX_OVERRIDE=... nixos-rebuild ...` if you rebuild that way.
