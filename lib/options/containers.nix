...
in
{
  options.containers = lib.mkOption {
    ...
    description = ''
      ...
      from one, see `packages.<host>-proxmox-lxc`) and carries users, aspects and a
        ...
        { name, ... }:
        {
          options = {
            description = lib.mkOption {
              type = t.nullOr t.str;
              default = null;
              description = "One line on what this image is for.";
            };
            ...
            system = lib.mkOption {
              type = t.enum config.aspectLib.systems;
